...
const T = {
  ar: {
    ...
    pageTitle: 'تطوير تطبيقات الموبايل',
    pageSub: 'أخبرنا عن تطبيقك وسنتواصل معك في أقرب وقت.',
    ...
    platformOpts: ['Android فقط', 'iOS فقط', 'Android + iOS (كلاهما)'],
    projectType: 'نوع التطبيق', projectTypePh: 'اختر نوع التطبيق',
    projectTypes: ['تجارة إلكترونية / متجر', 'تطبيق خدمات', 'تطبيق اجتماعي', 'تطبيق أداة / إنتاجية', 'تطبيق تعليمي', 'تطبيق حجوزات', 'أخرى'],
    features: 'الميزات المطلوبة (اختر كل ما ينطبق)',
    featuresList: ['تسجيل دخول / حسابات', 'إشعارات Push', 'خرائط / موقع جغرافي', 'دفع إلكتروني', 'محادثة / Chat', 'كاميرا / رفع صور', 'وضع offline', 'لوحة تحكم للإدارة'],
    ...
    budgetOpts: ['أقل من 30,000 دج', '30,000 — 70,000 دج', '70,000 — 150,000 دج', '150,000 دج فأكثر', 'للتفاوض'],
    timeline: 'المدة المطلوبة للإنجاز',
    timelineOpts: ['في أقرب وقت ممكن', 'خلال شهرين', '2 — 4 أشهر', 'أكثر من 4 أشهر'],
    description: 'وصف التطبيق',
    descriptionPh: 'اشرح فكرة تطبيقك — الجمهور المستهدف، المشكلة التي يحلها، أي مرجع تستلهم منه...',
    ...
    selectPlatform: 'الرجاء اختيار المنصة',
    selectType: 'الرجاء اختيار نوع التطبيق',
  ...
  fr: {
    ...
    pageTitle: 'Développement Mobile',
    pageSub: 'Parlez-nous de votre application et nous vous contacterons rapidement.',
    ...
    platformOpts: ['Android seulement', 'iOS seulement', 'Android + iOS (les deux)'],
    projectType: 'Type d\'application', projectTypePh: 'Choisir le type',
    projectTypes: ['E-commerce / boutique', 'Application de services', 'Application sociale', 'Outil / productivité', 'Application éducative', 'Réservation', 'Autre'],
    features: 'Fonctionnalités souhaitées (tout ce qui s\'applique)',
    featuresList: ['Connexion / comptes', 'Notifications Push', 'Cartes / géolocalisation', 'Paiement en ligne', 'Chat / messagerie', 'Caméra / upload photos', 'Mode hors-ligne', 'Panel d\'administration'],
    ...
    budgetOpts: ['Moins de 30 000 DA', '30 000 — 70 000 DA', '70 000 — 150 000 DA', '150 000 DA et plus', 'À négocier'],
    timeline: 'Délai souhaité',
    timelineOpts: ['Dès que possible', 'Dans 2 mois', '2 — 4 mois', 'Plus de 4 mois'],
    description: 'Description de l\'application',
    descriptionPh: 'Décrivez l\'idée de votre application — public cible, problème résolu, références...',
    ...
    selectPlatform: 'Veuillez choisir une plateforme',
  ...
  en: {
    ...
    pageSub: 'Tell us about your app and we\'ll reach out shortly.',
    ...
    platformOpts: ['Android only', 'iOS only', 'Android + iOS (both)'],
    projectType: 'App type', projectTypePh: 'Select app type',
    projectTypes: ['E-commerce / store', 'Service app', 'Social app', 'Tool / productivity', 'Educational app', 'Booking app', 'Other'],
    features: 'Required features (select all that apply)',
    featuresList: ['Login / user accounts', 'Push notifications', 'Maps / geolocation', 'Online payment', 'Chat / messaging', 'Camera / photo upload', 'Offline mode', 'Admin panel'],
    ...
    budgetOpts: ['Less than 30,000 DA', '30,000 — 70,000 DA', '70,000 — 150,000 DA', '150,000 DA+', 'Negotiable'],
    timeline: 'Desired timeline',
    timelineOpts: ['As soon as possible', 'Within 2 months', '2 — 4 months', 'More than 4 months'],
    description: 'App description',
    descriptionPh: 'Describe your app idea — target audience, problem it solves, references...',
    ...
    selectType: 'Please select an app type',
...
interface FormData {
  ...
  platform: string; projectType: string; features: string[];
  hasDesign: string; budget: string; timeline: string; description: string;
}

const BLANK: FormData = {
  fullName: '', phone: '', email: '', wilaya: '',
  platform: '', projectType: '', features: [],
  hasDesign: '', budget: '', timeline: '', description: '',
}
...
export default function AppDevPage() {
  ...
  function validateStep1(): boolean {
    ...
    if (!form.platform)    e.platform    = t.selectPlatform
  ...
  const inputCls = (err?: string) =>
    `w-full px-4 py-3 rounded-xl border text-sm outline-none transition-all duration-200 bg-white
     ${err ? 'border-red-400 focus:ring-2 focus:ring-red-200' : 'border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100'}`
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-lg p-10 max-w-md w-full text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button onClick={() => { setForm(BLANK); setStep(1); setDone(false) }}
              className="flex-1 inline-flex items-center justify-center px-6 py-3 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors">
              {t.newOrder}
            </button>
          </div>
        </div>
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} className={`min-h-screen bg-slate-50/50 flex flex-col ${isRTL ? 'font-arabic' : 'font-sans'}`}>
      ...
      <main className="flex-1 w-full max-w-2xl mx-auto px-4 sm:px-6 py-10 md:py-16">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400 font-semibold mb-8">
          <Link href="/" className="hover:text-indigo-600 transition-colors">{t.home}</Link>
          <span>/</span>
          <Link href="/order/professional" className="hover:text-indigo-600 transition-colors">{t.pro}</Link>
          <span>/</span>
          <span className="text-indigo-600">{t.pageTitle}</span>
        </div>
          ...
          {[t.step1, t.step2, t.step3].map((label, idx) => {
            ...
            return (
              <div key={n} className="flex items-center flex-1 last:flex-initial">
                ...
                {idx < 2 && (
                  <div className={`flex-1 h-0.5 mx-2 mb-4 transition-colors duration-300 ${step > n ? 'bg-indigo-600' : 'bg-slate-200'}`} />
                )}
              </div>
            )
          })}
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input value={form.email} onChange={e => set('email', e.target.value)}
                  placeholder={t.emailPh} type="email" className={inputCls()} />
              </div>
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              {/* Platform */}
              <div>
                <label className={labelCls}>{t.platform} <span className="text-red-500">*</span></label>
                <div className="flex flex-col gap-2">
                  {t.platformOpts.map((opt: string) => (
                    <button key={opt} type="button" onClick={() => set('platform', opt)}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium text-start transition-all duration-200
                        ${form.platform === opt ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0
                        ${form.platform === opt ? 'border-indigo-600' : 'border-slate-300'}`}>
                        {form.platform === opt && <div className="w-2.5 h-2.5 rounded-full bg-indigo-600" />}
                      </div>
                      {opt}
                    </button>
                  ))}
                  ...
                  {t.featuresList.map((feat: string) => {
                    ...
                    return (
                      <button key={feat} type="button" onClick={() => toggleFeature(feat)}
                        className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium text-start transition-all duration-200
                          ${checked ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {t.hasDesignOpts.map((opt: string) => (
                    <button key={opt} type="button" onClick={() => set('hasDesign', opt)}
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl border text-sm font-medium text-start transition-all duration-200
                        ${form.hasDesign === opt ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0
                        ${form.hasDesign === opt ? 'border-indigo-600' : 'border-slate-300'}`}>
                        {form.hasDesign === opt && <div className="w-2.5 h-2.5 rounded-full bg-indigo-600" />}
                      </div>
                      {opt}
                    </button>
                  ))}
                  ...
                  {t.budgetOpts.map((opt: string) => (
                    <button key={opt} type="button" onClick={() => set('budget', opt)}
                      className={`px-4 py-3 rounded-xl border text-sm font-medium text-start transition-all duration-200
                        ${form.budget === opt ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {t.timelineOpts.map((opt: string) => (
                    <button key={opt} type="button" onClick={() => set('timeline', opt)}
                      className={`px-4 py-3 rounded-xl border text-sm font-medium text-start transition-all duration-200
                        ${form.timeline === opt ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
                      {opt}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea value={form.description} onChange={e => set('description', e.target.value)}
                  placeholder={t.descriptionPh} rows={4} className={`${inputCls()} resize-none`} />
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              {[
                ...
                { label: t.platform,    val: form.platform },
              ...
              {form.features.length > 0 && (
                <div className="py-2.5 border-b border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {form.features.map(f => (
                      <span key={f} className="text-[11px] font-semibold bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-lg">{f}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className={`flex gap-3 mt-8 ${step === 1 ? 'justify-end' : 'justify-between'}`}>
            ...
            {step < 3 ? (
              <button onClick={handleNext}
                className="px-8 py-3 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 transition-colors shadow-sm">
                {t.next}
              </button>
            ) : (
              <button onClick={handleSubmit} disabled={loading}
                className="px-8 py-3 rounded-xl bg-indigo-600 text-white font-semibold text-sm hover:bg-indigo-700 disabled:opacity-60 disabled:cursor-not-allowed transition-colors shadow-sm">
