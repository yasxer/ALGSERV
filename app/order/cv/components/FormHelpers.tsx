...
export function FormSection({ title, children, helpText, helpTextAr }: { title: string; children: React.ReactNode; helpText?: string; helpTextAr?: string }) {
      ...
      {showHelp && (helpText || helpTextAr) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={() => setShowHelp(false)}>
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl" dir="rtl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-ink-700 leading-relaxed text-right whitespace-pre-line">{helpTextAr ?? helpText}</p>
